...
def filter_clients(all_clients, search_text, field):
    return [
        ...
        if search_text.lower() in str(client.get(field, "")).lower()
    ]
...
def menu_page(page: ft.Page):
    ...
    search_field_razao = ft.TextField(
    ...
    if clients_data.each():
        for client in clients_data.each():
            ...
            if client_info:
                ...
                client_razao = client_info.get("RAZAO", "")
...
def open_add_client_page(page: ft.Page, client_data=None):
    ...
    rdp_ip_value = client_data["RDP_IP"] if client_data else ""
    rdp_user_value = client_data["RDP_USER"] if client_data else ""
    rdp_pass_value = client_data["RDP_PASS"] if client_data else ""
    ...
    add_client_dialog = ft.AlertDialog(
        ...
        content=ft.Column(
            controls=[
                ft.TextField(label="Nome", width=300, value=nome_value, fill_color= '#06140f'),
                ft.TextField(label="Razão", width=300, value=razao_value, fill_color= '#06140f'),
                ft.TextField(label="CNPJ", width=300, value=cnpj_value, fill_color= '#06140f'),
                ft.TextField(label="Conexão", width=300, value=conexao_value, fill_color= '#06140f'),
                ft.TextField(label="Senha", width=300, password=True, value=senha_value, fill_color= '#06140f'),
                ft.TextField(label="IP", width=300, value=ip_value, fill_color= '#06140f'),
                ft.TextField(label="Porta", width=300, value=porta_value, fill_color= '#06140f'),
                ft.TextField(label="RDP IP", width=300, value=rdp_ip_value, fill_color= '#06140f'),
                ft.TextField(label="RDP Usuário", width=300, value=rdp_user_value, fill_color= '#06140f'),
                ft.TextField(label="RDP Senha", width=300, password=True, value=rdp_pass_value, fill_color= '#06140f'),
            ],
            ...
            ft.Row(
                controls=[
                    ft.ElevatedButton(
                        ...
                        width=150,style=ft.ButtonStyle(bgcolor="#CC8105")
                    ),
                    ft.TextButton(
                        text="Voltar",
                        on_click=lambda e: close_dialog(page, add_client_dialog),
                        width=150,style=ft.ButtonStyle(bgcolor="#7B0000")
                    ),
                ],
                alignment=ft.MainAxisAlignment.CENTER
            )
...
def save_client(event, page: ft.Page, dialog: ft.AlertDialog):
    ...
    client_data = {
        ...
        "RDP_IP": fields[7].value,
        "RDP_USER": fields[8].value,
        "RDP_PASS": fields[9].value
    }
